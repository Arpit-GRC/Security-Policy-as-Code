...
•	And real security issues slip through the cracks
...
AWS introduced Service Control Policies, IAM guardrails, Config rules, Access Analyzer, and OPA/Rego integrations.
Suddenly, we gained the ability to:
•	Turn policies into automated rules
...
This project explores how real-world information security controls—ISO 27002, SOC 2, NIST, CIS Benchmark—
can be implemented not as paperwork, but as living, executable rules inside AWS.
...
•	Build guardrails that developers can trust
...
→ A bucket policy denies the upload.
When a policy says:
•	“Privileged access must be restricted and reviewed”
→ Identity Center, CloudTrail, and guardrails make it happen automatically.
